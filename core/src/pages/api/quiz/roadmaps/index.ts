...
import {
  createRoadmap,
  fetchRoadmaps,
  type FetchRoadmapsInput,
} from '../../../../lib/quiz/roadmaps';
...
export const GET: APIRoute = async ({ request }) => {
  try {
    ...
    const params: Partial<FetchRoadmapsInput> = {};
    ...
    if (filters) params.filters = filters as FetchRoadmapsInput['filters'];

    const sort = parseSort(url.searchParams);
    if (sort) params.sort = sort as FetchRoadmapsInput['sort'];

    const result = await fetchRoadmaps(params);
    return json(result);
  } catch (error) {
