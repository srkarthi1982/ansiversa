...
export const GET: APIRoute = async ({ request }) => {
  try {
    ...
    const params: Partial<FetchTopicsInput> = {};
    ...
    if (filters) params.filters = filters as FetchTopicsInput['filters'];

    const sort = parseSort(url.searchParams);
    if (sort) params.sort = sort as FetchTopicsInput['sort'];
