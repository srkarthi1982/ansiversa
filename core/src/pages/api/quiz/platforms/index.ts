...
import {
  createPlatform,
  fetchPlatforms,
  type FetchPlatformsInput,
} from '../../../../lib/quiz/platforms';
...
const parseFilters = (searchParams: URLSearchParams) => {
  ...
  const stringFields: Array<'name' | 'description' | 'type' | 'status'> = [
    'name',
    'description',
    'type',
    'status',
...
export const GET: APIRoute = async ({ request }) => {
  try {
    ...
    const params: Partial<FetchPlatformsInput> = {};
    ...
    if (filters) params.filters = filters as FetchPlatformsInput['filters'];

    const sort = parseSort(url.searchParams);
    if (sort) params.sort = sort as FetchPlatformsInput['sort'];

    const result = await fetchPlatforms(params);
    return json(result);
  } catch (error) {
...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    const platform = await createPlatform(payload);
    return json(platform, { status: 201 });
  } catch (error) {
    return handleError(error);
  }
};
