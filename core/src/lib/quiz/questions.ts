...
type NormalizedQuestionRow = QuestionRow & {
...
const normalizeQuestion = (row: NormalizedQuestionRow) => {
...
export const fetchQuestionsInputSchema = z.object({
...
export type FetchQuestionsInput = z.infer<typeof fetchQuestionsInputSchema>;
export type QuestionPayload = z.infer<typeof questionPayloadSchema>;

export async function fetchQuestions(input: unknown) {
  const { page, pageSize, filters, sort } = await fetchQuestionsInputSchema.parseAsync(input ?? {});
  ...
  if (normalizedFilters.questionText) {
    conditions.push(sql`lower(${Question.q}) LIKE ${`%${normalizedFilters.questionText.toLowerCase()}%`}`);
  }
...
export async function createQuestion(input: unknown) {
  const payload = normalizeInput(await questionPayloadSchema.parseAsync(input));
  ...
  const roadmapRow = await roadmapRepository.getById((table) => table.id, payload.roadmapId);
  if (!roadmapRow) {
    throw new ActionError({ code: 'BAD_REQUEST', message: 'Roadmap not found' });
  }

  if (
    roadmapRow.platformId !== payload.platformId ||
    roadmapRow.subjectId !== payload.subjectId ||
    roadmapRow.topicId !== payload.topicId
  ) {
    throw new ActionError({
      code: 'BAD_REQUEST',
...
const updateQuestionSchema = z.object({
  ...
  data: questionPayloadSchema,
});

export type UpdateQuestionInput = z.infer<typeof updateQuestionSchema>;

export async function updateQuestion(input: unknown) {
  const { id, data } = await updateQuestionSchema.parseAsync(input ?? {});
...
const deleteQuestionSchema = z.object({ id: z.number().int().min(1) });

export type DeleteQuestionInput = z.infer<typeof deleteQuestionSchema>;

export async function deleteQuestion(input: unknown) {
  const { id } = await deleteQuestionSchema.parseAsync(input ?? {});
  ...
  try {
    ...
    throw new ActionError({
      code: 'BAD_REQUEST',
      message: (err as Error)?.message ?? 'Unable to delete question',
    });
  }
}

const singleQuestionSchema = z.object({ id: z.number().int().min(1) });

export async function getQuestionById(input: unknown) {
  const { id } = await singleQuestionSchema.parseAsync(input ?? {});
  const record = await questionQueryRepository.getById((table) => table.id, id);
  ...
  return normalizeQuestion({
    ...
    roadmapName: record.roadmapName ?? null,
  });
}
