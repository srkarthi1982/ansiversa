...
export const fetchPlatformsInputSchema = z.object({
...
export type FetchPlatformsInput = z.infer<typeof fetchPlatformsInputSchema>;
export type PlatformPayload = z.infer<typeof platformPayloadSchema>;

export async function fetchPlatforms(input: unknown) {
  const { page, pageSize, filters, sort } = await fetchPlatformsInputSchema.parseAsync(
    input ?? {},
  );
...
export async function createPlatform(input: unknown) {
  const payload = normalizeInput(await platformPayloadSchema.parseAsync(input));
...
export async function updatePlatform(input: unknown) {
  const { id, data } = await updatePlatformSchema.parseAsync(input ?? {});
...
const deletePlatformSchema = z.object({ id: z.number().int().min(1) });

export type DeletePlatformInput = z.infer<typeof deletePlatformSchema>;

const singlePlatformSchema = z.object({ id: z.number().int().min(1) });

export async function getPlatformById(input: unknown) {
  const { id } = await singlePlatformSchema.parseAsync(input ?? {});
  const record = await platformRepository.getById((table) => table.id, id);
  if (!record) {
    throw new ActionError({ code: 'NOT_FOUND', message: 'Platform not found' });
  }
...
export async function deletePlatform(input: unknown) {
  const { id } = await deletePlatformSchema.parseAsync(input ?? {});
