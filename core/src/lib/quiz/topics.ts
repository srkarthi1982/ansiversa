...
export const fetchTopicsInputSchema = z.object({
...
export type FetchTopicsInput = z.infer<typeof fetchTopicsInputSchema>;
export type TopicPayload = z.infer<typeof topicPayloadSchema>;

export async function fetchTopics(input: unknown) {
  const { page, pageSize, filters, sort } = await fetchTopicsInputSchema.parseAsync(input ?? {});
...
export async function createTopic(input: unknown) {
  const payload = normalizeInput(await topicPayloadSchema.parseAsync(input));
...
export async function updateTopic(input: unknown) {
  const { id, data } = await updateTopicSchema.parseAsync(input ?? {});
...
const deleteTopicSchema = z.object({ id: z.number().int().min(1) });

export type DeleteTopicInput = z.infer<typeof deleteTopicSchema>;

export async function deleteTopic(input: unknown) {
  const { id } = await deleteTopicSchema.parseAsync(input ?? {});
...
const singleTopicSchema = z.object({ id: z.number().int().min(1) });

export async function getTopicById(input: unknown) {
  const { id } = await singleTopicSchema.parseAsync(input ?? {});
  const record = await topicQueryRepository.getById((table) => table.id, id);
