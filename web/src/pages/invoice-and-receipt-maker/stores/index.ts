...
class InvoiceStore extends BaseStore {
  ...
  onInit(location?: Location) {
    ...
    if (pathname.startsWith('/invoice-and-receipt-maker/builder')) {
      const params = new URL(location?.href ?? window.location.href).searchParams;
      const id = params.get('id');
      this.initBuilder({ id });
      return;
    }
    if (pathname.startsWith('/invoice-and-receipt-maker/view/')) {
      const slug = pathname.split('/').filter(Boolean).at(-1) ?? '';
      this.initViewer(slug);
      return;
    }
    if (pathname.startsWith('/invoice-and-receipt-maker/receipt/')) {
      const receiptId = pathname.split('/').filter(Boolean).at(-1) ?? '';
      this.initReceipt(receiptId);
      return;
    }
    if (pathname.startsWith('/invoice-and-receipt-maker/templates')) {
      if (!this.isLoaded) this.loadFromStorage();
      return;
    }
    if (pathname.startsWith('/invoice-and-receipt-maker')) {
      this.initDashboard();
    }
  }
  ...
  createDraft() {
    ...
    if (typeof window !== 'undefined') {
      const url = `/invoice-and-receipt-maker/builder?id=${invoice.id}`;
      if (window.location.pathname.startsWith('/invoice-and-receipt-maker/builder')) {
  ...
  markSent() {
    ...
    if (typeof window !== 'undefined') {
      this.builder.record.shareUrl = `${window.location.origin}/invoice-and-receipt-maker/view/${slug}`;
    }
