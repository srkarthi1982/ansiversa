...
class EmailStore extends BaseStore {
  ...
  async polish() {
    ...
    try {
      const { data, error } = await actions.email.polish({
  ...
  async rewrite(mode: EmailRewriteMode) {
    ...
    this.quick.loading = true;
    try {
      const { data, error } = await actions.email.rewrite({
        text: source,
        mode,
        tone: this.quick.tone,
        language: this.quick.language,
  ...
  async translateOutput() {
    ...
    try {
      const { data, error } = await actions.email.translate({
        text: source,
  ...
  async generateReplies() {
    ...
    try {
      const { data, error } = await actions.email.reply({
  ...
  async summarizeThread() {
    ...
    try {
      ...
      const { data, error } = await actions.email.summarize({
  ...
  async useTemplate(template: EmailTemplate) {
    ...
    this.quick.error = null;
    try {
      const { data, error } = await actions.email.renderTemplate({
  ...
  async saveDraft(status: 'draft' | 'final' = 'draft') {
    ...
    try {
      ...
      const { data, error } = await actions.email.save({ id: this.drafts.activeId, draft: payload });
      if (error) throw error;
  ...
  openEditor() {
    if (this.drafts.activeId) {
      window.location.assign(`/email/editor?id=${this.drafts.activeId}`);
    } else {
