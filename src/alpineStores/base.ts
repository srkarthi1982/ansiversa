...
type AlpineGlobal = typeof globalThis & {
  Alpine?: {
    store?: (name: string, value?: unknown) => unknown;
  };
};

function getLoaderStore(): LoaderLike | undefined {
  const alpine = (globalThis as AlpineGlobal).Alpine;
  if (!alpine || typeof alpine.store !== 'function') {
    return undefined;
  }

  try {
    return alpine.store('loader') as LoaderLike | undefined;
  } catch (error) {
    console.warn('Unable to resolve loader store', error);
    return undefined;
  }
}
...
export function setLoaderVisible(visible: boolean): void {
  if (visible) {
    showLoader();
  } else {
    hideLoader();
...
export function flashLoader(duration = 300): void {
  const loader = getLoaderStore();
  if (!loader) {
    return;
  }
  ...
  loader.show?.call(loader);
  setTimeout(() => {
    const currentLoader = getLoaderStore();
    currentLoader?.hide?.call(currentLoader);
  }, duration);
}

export async function withLoader<T>(operation: () => Promise<T>): Promise<T> {
  showLoader();
  try {
