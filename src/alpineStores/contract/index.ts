...
class ContractStore {
  ...
  onInit(location?: Location) {
    ...
    if (pathname.startsWith('/contract/builder')) {
      return;
    }
    if (pathname.startsWith('/contract/wizard')) {
      this.resetWizard();
      return;
    }
  ...
  async createDraft(templateKey?: ContractTemplateKey) {
    try {
      ...
      if (data?.contract) {
        ...
        this.applyFilters();
        window.location.assign(`/contract/builder?id=${contract.id}`);
      }
    } catch (error) {
  ...
  async initBuilder(options: BuilderInitInput) {
    ...
    try {
      ...
      if (options.id) {
        const { data, error } = await actions.contract.get({ id: options.id });
        if (error) throw error;
        if (data?.contract) {
        ...
        if (data?.contract) {
          ...
          window.history.replaceState({}, '', `/contract/builder?id=${contract.id}`);
        }
      }
    } catch (error) {
  ...
  setBuilderRecord(record: ContractRecord) {
    ...
    this.builder.shareUrl = record.slug ? `/contract/view/${record.slug}` : null;
  ...
  async saveNow() {
    ...
    try {
      ...
      const { data, error } = await actions.contract.save(payload);
      if (error) throw error;
      if (data?.contract) {
  ...
  async loadClauseLibrary() {
    ...
    try {
      const { data, error } = await actions.contract.library({});
      if (error) throw error;
  ...
  async runClauseAI(target: keyof ContractClauses, mode: 'tighten' | 'draft' | 'translate' | 'simplify') {
    ...
    try {
      ...
      const { data, error } = await actions.contract.aiClause({
  ...
  async completeWizard() {
    ...
    this.wizard.error = null;
    try {
      const { data, error } = await actions.contract.create({ templateKey: this.wizard.data.templateKey });
      if (error) throw error;
