...
const minutesActions = actions['meeting-minutes-ai'];
...
class MinutesStore extends BaseStore {
  ...
  onInit(location?: Location) {
    ...
    if (pathname.startsWith('/meeting-minutes-ai/builder')) {
      const params = new URLSearchParams(location?.search ?? window.location.search);
      const id = params.get('id');
      this.initBuilder({ id });
      return;
    }
    if (pathname.startsWith('/meeting-minutes-ai/templates')) {
      this.initTemplates();
      return;
    }
    if (pathname.startsWith('/meeting-minutes-ai/actions')) {
      this.initActions();
      return;
    }
    this.initDashboard();
  ...
  async loadList() {
    ...
    this.loader?.show();
    try {
      const { data, error } = await minutesActions.list({});
  ...
  async createDraft(templateKey?: MinutesTemplateKey) {
    try {
      this.loader?.show();
      const { data, error } = await minutesActions.create({ templateKey });
      if (error) throw error;
  ...
  async loadBuilder(id: string | null) {
    ...
    try {
      ...
      const { data, error } = await minutesActions.get({ id: targetId });
      if (error) throw error;
  ...
  private populateBuilder(record: MinutesRecord) {
    ...
    this.builder.shareUrl = record.slug ? `/meeting-minutes-ai/view/${record.slug}` : null;
  }

  private async persistBuilder(payload: Record<string, unknown>) {
    if (!this.builder.id) return;
    try {
      const { data, error } = await minutesActions.save({
        id: this.builder.id,
        data: payload,
      });
      if (error) throw error;
  ...
  async summarize() {
    ...
    try {
      const { data, error } = await minutesActions.summarize({ id: this.builder.id });
      if (error) throw error;
  ...
  async transcribe(durationSec = 120) {
    ...
    try {
      const { data, error } = await minutesActions.transcribe({
        id: this.builder.id,
        durationSec,
      });
      if (error) throw error;
  ...
  async export(format: 'pdf' | 'docx' | 'md' | 'csv') {
    ...
    try {
      const { data, error } = await minutesActions.export({ id: this.builder.id, format });
      if (error) throw error;
  ...
  async publish() {
    if (!this.builder.id) return;
    try {
      const { data, error } = await minutesActions.publish({ id: this.builder.id });
      if (error) throw error;
      const slug = data?.slug as string | undefined;
      if (slug) {
        this.builder.shareUrl = `/meeting-minutes-ai/view/${slug}`;
  ...
  async delete(id: string) {
    ...
    try {
      const { error } = await minutesActions.delete({ id });
      if (error) throw error;
