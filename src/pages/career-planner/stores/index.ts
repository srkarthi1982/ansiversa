...
class CareerStore extends BaseStore {
  ...
  onInit(location: Location) {
    if (location.pathname.startsWith('/career-planner/builder')) {
      const params = new URLSearchParams(location.search);
      this.initBuilder({ id: params.get('id') });
    } else if (location.pathname.startsWith('/career-planner/tracker')) {
      this.initTracker();
    } else if (location.pathname.startsWith('/career-planner/roles')) {
      this.initRoles();
    } else if (location.pathname.startsWith('/career-planner/view/')) {
      const slug = location.pathname.split('/').filter(Boolean).pop() ?? '';
      this.initPublicView(slug);
    } else if (location.pathname.startsWith('/career-planner')) {
      this.initDashboard();
    }
  }
  ...
  async createPlan(title?: string): Promise<void> {
    ...
    try {
      ...
      const response = await this.callCareerApi<{ plan: CareerPlanSummary; redirectTo: string }>('/career-planner/api/create', body);
      if (!response) return;
  ...
  async deletePlan(planId: string): Promise<void> {
    ...
    try {
      await this.callCareerApi<{ ok: boolean }>('/career-planner/api/delete', { id: planId });
      if (this.dashboard.data) {
  ...
  async saveBuilderDraft(): Promise<void> {
    try {
      ...
      const result = await this.callCareerApi<{ lastSavedAt: string }>('/career-planner/api/save', {
        id: this.builder.id,
        payload: {
  ...
  async generateLearningPlan(): Promise<void> {
    try {
      const plan = await this.callCareerApi<{ learningPlan: CareerLearningPlan; etaWeeks: number }>(
        '/career-planner/api/plan/learning',
        {
          skillsCurrent: this.builder.skillsCurrent,
          skillsRequired: this.builder.skillsRequired,
  ...
  async generateSprintPlan(): Promise<void> {
    try {
      const result = await this.callCareerApi<{ sprintPlan: CareerSprintPlan }>(
        '/career-planner/api/plan/sprint',
  ...
  async generateResumeBullets(): Promise<void> {
    try {
      const result = await this.callCareerApi<{ bullets: string[] }>(
        '/career-planner/api/resume-bullets',
        {
          profile: this.builder.profile,
          projects: this.builder.projects,
  ...
  async publishPlan(): Promise<void> {
    try {
      const data = await this.callCareerApi<{ url: string }>('/career-planner/api/publish', { id: this.builder.id });
      if (data?.url) {
  ...
  async exportPlan(format: 'pdf' | 'docx' | 'md' | 'csv'): Promise<void> {
    try {
      const data = await this.callCareerApi<{ url: string }>('/career-planner/api/export', { id: this.builder.id, format });
      if (data?.url) {
  ...
  async upsertApplication(application: CareerApplication): Promise<void> {
    try {
      const result = await this.callCareerApi<{ item: CareerApplication }>(
        '/career-planner/api/applications',
