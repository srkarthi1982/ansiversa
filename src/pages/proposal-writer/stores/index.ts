...
const proposalActions = actions['proposal-writer'];
...
class ProposalStore extends BaseStore {
  ...
  onInit(location?: Location) {
    ...
    if (pathname.startsWith('/proposal-writer/builder')) {
      return;
    }
    if (pathname.startsWith('/proposal-writer')) {
      this.initDashboard();
    }
  }
  ...
  async loadList() {
    ...
    this.loader?.show();
    try {
      const { data, error } = await proposalActions.list({});
  ...
  async createDraft(templateKey?: ProposalTemplateKey) {
    try {
      this.loader?.show();
      const { data, error } = await proposalActions.create(templateKey ? { templateKey } : {});
      if (error) throw error;
      if (data?.proposal) {
        const proposal = data.proposal as ProposalRecord;
        this.state.proposals.unshift(proposal);
        this.applyFilters();
        window.location.assign(`/proposal-writer/builder?id=${proposal.id}`);
      }
    } catch (error) {
  ...
  async duplicate(id: string) {
    try {
      this.loader?.show();
      const { data, error } = await proposalActions.duplicate({ id });
      if (error) throw error;
      if (data?.proposal) {
  ...
  async delete(id: string) {
    ...
    try {
      this.loader?.show();
      const { error } = await proposalActions.delete({ id });
      if (error) throw error;
  ...
  async initBuilder(options: BuilderInitInput) {
    ...
    try {
      if (options.id) {
        const { data, error } = await proposalActions.get({ id: options.id });
        if (error) throw error;
        if (data?.proposal) {
          this.setBuilderRecord(data.proposal as ProposalRecord);
        }
      } else {
        const { data, error } = await proposalActions.create({});
        if (error) throw error;
        if (data?.proposal) {
          this.setBuilderRecord(data.proposal as ProposalRecord);
          window.history.replaceState({}, '', `/proposal-writer/builder?id=${data.proposal.id}`);
        }
      }
    } catch (error) {
  ...
  setBuilderRecord(record: ProposalRecord) {
    ...
    this.builder.shareUrl = record.slug ? `/proposal-writer/view/${record.slug}` : null;
  ...
  async saveNow() {
    ...
    try {
      ...
      const { data, error } = await proposalActions.save(payload);
      if (error) throw error;
      if (data?.proposal) {
  ...
  async publishCurrent() {
    ...
    try {
      this.loader?.show();
      const { data, error } = await proposalActions.publish({ id: this.builder.id });
      if (error) throw error;
      if (data?.proposal) {
        const record = data.proposal as ProposalRecord;
  ...
  async publishFromList(id: string) {
    try {
      this.loader?.show();
      const { data, error } = await proposalActions.publish({ id });
      if (error) throw error;
      if (data?.proposal) {
        const record = data.proposal as ProposalRecord;
        this.upsertListRecord(record);
  ...
  async export(format: 'pdf' | 'docx' | 'md') {
    if (!this.builder.id) return;
    try {
      const { data, error } = await proposalActions.export({ id: this.builder.id, format });
      if (error) throw error;
      if (data?.url) {
  ...
  async runAIDraft(input: { client: string; industry?: string; services?: string[] }) {
    ...
    try {
      ...
      const { data, error } = await proposalActions.aiDraft(payload);
      if (error) throw error;
      if (data) {
        const next = clone(this.builder.data);
  ...
  async improveSection(text: string) {
    ...
    this.builder.aiBusy = true;
    try {
      const { data, error } = await proposalActions.aiImprove({ text, tone: this.builder.aiTone });
      if (error) throw error;
  ...
  async analyzeBrief(text: string) {
    ...
    try {
      const { data, error } = await proposalActions.briefAnalyze({ text, industry: this.builder.data.client.industry });
      if (error) throw error;
      if (data) {
