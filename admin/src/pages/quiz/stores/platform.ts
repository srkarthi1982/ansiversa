...
const CORE_API_BASE_URL = (() => {
  const raw = import.meta.env.PUBLIC_CORE_API_BASE_URL ?? 'http://localhost:2000';
  return raw.endsWith('/') ? raw.replace(/\/+$/, '') : raw;
})();

const buildUrl = (path: string) => `${CORE_API_BASE_URL}${path.startsWith('/') ? path : `/${path}`}`;
...
class PlatformStoreImpl extends BaseStore {
  ...
  async loadPlatforms(page = this.page): Promise<void> {
    ...
    try {
      ...
      const data = await this.request<PlatformListResponse>(`/quiz/platforms${query ? `?${query}` : ''}`);
      const payload = data ?? { items: [], total: 0, page, pageSize: this.pageSize };
  ...
  private async createPlatform(): Promise<void> {
    ...
    try {
      ...
      await this.request<PlatformRecord>('/quiz/platforms', {
        method: 'POST',
  ...
  private async saveEdit(): Promise<void> {
    ...
    try {
      const payload = this.toPayload(this.form);
      await this.request<PlatformRecord>(`/quiz/platforms/${this.editingId}`, {
        method: 'PUT',
        body: JSON.stringify(payload),
      });
  ...
  async deletePlatform(id: number): Promise<void> {
    ...
    this.error = null;
    try {
      await this.request(`/quiz/platforms/${id}`, { method: 'DELETE' });
  ...
  private async request<T>(path: string, init: RequestInit = {}): Promise<T> {
    ...
    if (!response.ok) {
      const message = (payload as { error?: string })?.error ?? response.statusText ?? 'Request failed';
